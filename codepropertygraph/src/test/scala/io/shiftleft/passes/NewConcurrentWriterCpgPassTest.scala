...
class NewConcurrentWriterCpgPassTest extends AnyWordSpec with Matchers {

  private object Fixture {
    def apply(
      keyPools: Option[Iterator[KeyPool]] = None
    )(f: (Cpg, NewConcurrentWriterCpgPass[String]) => Unit): Unit = {
      val cpg  = Cpg.emptyCpg
      val pool = keyPools.flatMap(_.nextOption())
      class MyPass(cpg: Cpg) extends NewConcurrentWriterCpgPass[String](cpg, "MyPass", pool) {
        override def generateParts(): Array[String] = Array("foo", "bar")

        override def runOnPartForCpg(diffGraph: overflowdb.BatchedUpdate.DiffGraphBuilder, part: String): Unit = {
