...
class ConcurrentQueueWriterCpgPassTest extends AnyWordSpec with Matchers {

  private object Fixture {
    def apply(
      keyPools: Option[Iterator[KeyPool]] = None
    )(f: (Cpg, ConcurrentQueueWriterCpgPass[String]) => Unit): Unit = {
      val cpg  = Cpg.emptyCpg
      val pool = keyPools.flatMap(_.nextOption())
      class MyPass(cpg: Cpg) extends ConcurrentQueueWriterCpgPass[String](cpg, "MyPass", pool) {
        override def generateParts(): Array[String] = Array("foo", "bar")

        override def runOnPart(diffGraph: overflowdb.BatchedUpdate.DiffGraphBuilder, part: String): Unit = {
  ...
  "ConcurrentWriterCpgPass" should {
    ...
    "fail for schema violations" in {
      val cpg = Cpg.emptyCpg
      val pass = new ConcurrentQueueWriterCpgPass[String](cpg, "pass2") {
    ...
    "add NewNodes that are referenced in different parts only once" in {
      ...
      val pass = new ConcurrentWriterCpgPass[String](cpg, "pass2") {
        ...
        val call2again = NewCall().name("call2")

        override def generateParts() = Array("a", "b")
        override def runOnPart(diffGraph: DiffGraphBuilder, part: String): Unit =
          part match {
            case "a" =>
              diffGraph.addEdge(call2, call1, "AST")
            case "b" =>
              diffGraph.addEdge(call3, call2again, "AST")
          }
      }
      pass.createAndApply()
