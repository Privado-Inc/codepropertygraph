...
import io.shiftleft.passes.GenericConcurrentQueueProcessor.{STOP, zContext}
...
object GenericConcurrentQueueProcessor {
  // set maximum sockets that can be used. One can update this max count before any use of ConcurrentProcessor and the value will be picked up
  var maxSocketCount = 4096
  lazy val zContext = {
    val context = new ZContext()
    context.getContext.setMaxSockets(maxSocketCount)
    context
  }
  val STOP = "STOP"
}

trait BaseConcurrentQueueProcessor[T <: AnyRef, R] {

  protected val logger = LoggerFactory.getLogger(this.getClass)
  // PUSH socket maintained for each task thread (Future)
  protected val queue: ThreadLocal[ZMQ.Socket] = new ThreadLocal[ZMQ.Socket]
  // Unique ZeroMQ inproc:// topic used for each instance of ConcurrentProcessor.
  protected val topic: String = UUID.randomUUID().toString
  ...
  /** This method will be called from the Writer thread to write the results to the result object. How to write the
    ...
    * @param item
    *   \- one or more result object of invocation of the method {@link GenericConcurrentQueueProcessor# runOnPart ( )}
    *   for each parts. Which will be queued by calling {@link GenericConcurrentQueueProcessor# addInWriterQueue (
    *   command : String, item: Any)}
    * @param result
  ...
  protected def pushStopCommandInQueue(): Unit = {
    val queue = getQueue()
    queue.send(STOP)
    cleanup()
  }
  ...
  /** Factory object for PUSH socket of ZeroMQ, which will maintain separate instance per worker thread.
    ...
    *   \- ZeroMQ PUSH Socket.
    */
  protected def getQueue(): ZMQ.Socket = {
    val temp = queue.get()
    Option(temp) match {
      case Some(socket) =>
        socket
  ...
  /** Connect to ZeroMQ queue topic using PUSH socket and push the {@command} along with the {@item} to be processed
    ...
    * @param item
    *   \- result object generated from the processing of {@link GenericConcurrentQueueProcessor#runOnPart()}
    */
  protected def addInWriterQueue(command: String, item: Any): Unit = {
    try {
      val queue = getQueue()
  ...
  /** Serialise any object to byte array to be passed through queue
    ...
    */
  protected def serialise(value: Any): Array[Byte] = {
  ...
  /** Deserialize the ByteArray back to Object.
    ...
    *   \- Deserialized object
    */
  protected def deserialize(bytes: Array[Byte]): Any = {
  ...
  /** Independent writer thread which will listen to incoming message (command along with item to be processed) from the
    * PULL Socket queue.
    */
  protected class Writer(result: R) extends Runnable {
...
/** Concurrent processor to process the individual processing of the data in parallel threads for each item of the array
  * returned by {@link GenericConcurrentQueueProcessor#generateParts()}.
  *
  * This will be done by invoking {@link GenericConcurrentQueueProcessor#runOnPart()} method from independent Future by
  * passing each item of the array to it.
  *
  ...
  *
  * This can be done by calling {@link GenericConcurrentQueueProcessor#addInWriterQueue(command: String, item: Any)}
  * method
  *
  ...
  *
  * {@link GenericConcurrentQueueProcessor#processCommand(command: String, item: Any, result: R)} method implementation.
  *
  * @param result
  *   \- Result object which needs to be populated with the results.
  * @tparam T
  *   \- Type of the element which will be the source input to {@link GenericConcurrentQueueProcessor#runOnPart()}
  * @tparam R
  *   \- Type of the result object
  */
abstract class GenericConcurrentQueueProcessor[T <: AnyRef, R](result: R) extends BaseConcurrentQueueProcessor[T, R] {
  ...
  def process(): R = {
    ...
    Option(genParts) match {
      case Some(parts) if parts.size > 0 =>
        val queueWriter = new Thread(Writer(result))
        queueWriter.start()
        ConcurrentTaskUtil
          .runUsingThreadPool(parts.map(part => () => new TaskProcessor(part).call()).iterator)
          .map {
            case Failure(e) =>
              logger.warn("Exception encountered inside TaskProcessor", e)
            case _ =>
          }
